...
enum IP_ENUM_GHSTATIONINFO
{
    ...
    IP_MAX_NUMBER_BLOBS,
    IP_STATION_OVERLAY,
    IP_HEALTH_MONITORING_ON_OFF,
    IP_DEFAULT_HEALTH_SOURCE,
    IP_DEFAULT_HEALTH_REPORT,
    IP_DEFAULT_HEALTH_TIME_DELTA,
    IP_TESTING_DISK_LIMIT_ON_OFF,
    IP_TESTING_DISK_LIMIT_IN_MB,
...
enum IP_ENUM_BLOBPOLICY
{
    ...
    IP_2YEARS,          //retain for 2 years
    IP_3YEARS,          //retain for 3 years
    IP_4YEARS,          //retain for 4 years
    IP_5YEARS,          //retain for 5 years
    IP_6YEARS,          //retain for 6 years
    IP_7YEARS,          //retain for 7 years
    IP_8YEARS,          //retain for 8 years
    IP_9YEARS,          //retain for 9 years
    IP_10YEARS          //retain for 10 years
};
...
#define IP_ATTRIBUTE_HEAD_ID                    "head_id"//"head id for pdca"
#define IP_ATTRIBUTE_FIXTURE_ID                 "fixture_id"//"fixture id"
#define IP_ATTRIBUTE_SWVERSION		            "sw_version"//"software_version"
#define IP_ATTRIBUTE_ACORN_TYPE					"acorn_type"//"acorn_type"
#define IP_ATTRIBUTE_TEST_HEAD_ID               "test_head_id"//"test head id for sfc"
...
//Please see the document/wiki page for the correct naming conventions
//https://hwtewiki.apple.com/display/dc/PDCA+Data+Structure+Definition
//https://hwtewiki.apple.com/download/attachments/9765073/PDCA%20Special%20Build%20SOP%20v1.2%2020110117.key?version=1&modificationDate=1375813243000&api=v2
...
#define IP_ATTRIBUTE_SPECIAL_BUILD				"S_BUILD"				//ex: N82-PPVT_R4_120 ( build_config_unitnumber )
...
//mac equipment ids
#define IP_ATTRIB_EQUIPMENT_ID				    "equipment_id"
#define IP_ATTRIB_FW_VERSION					"fw_version"
#define IP_ATTRIB_TYPE						    "type"
...
//Misc defines
#define IP_NOVALUE          "N/A"
#define IP_RETVALUE         "RetValue"

/// PDCA priority definitions
enum IP_PDCA_PRIORITY
{
    IP_PRIORITY_STATION_CALIBRATION_AUDIT	= -2,  //audit or station calibration
    IP_PRIORITY_REALTIME_WITH_ALARMS		= 0,   //quality critical keys
    IP_PRIORITY_REALTIME					= 1,   //mass production keys
    IP_PRIORITY_DELAYED_WITH_DAILY_ALARMS	= 2,   //med-res keys
    IP_PRIORITY_DELAYED_IMPORT				= 3,   //hi-res keys
    IP_PRIORITY_ARCHIVE						= 4,   //development data
    IP_PRIORITY_INFO_ONLY					= 5,   //info only
...
enum IP_MSG_NUMBER
{
    ...
    IP_MSG_ERROR_SUPPLANT_APICALL,
    IP_MSG_ERROR_NOT_ENOUGH_HARDISK_SPACE,
    IP_MSG_ERROR_INVALID_HASH,
    IP_MSG_ERROR_INVALID_TEST_START_TIME,
    IP_MSG_ERROR_INVALID_TEST_END_TIME,
...
extern "C" {
    ...
    void IP_UID_destroy( IP_UUTHandle inHandle );
    ...
    ///@param inHandle : the handle originally returned from IP_UUTStart()
    IP_API_Reply IP_addResult( IP_UUTHandle inHandle, IP_TestSpecHandle testSpecHandle, IP_TestResultHandle testResultHandle );
    
    ///Add an attribute for your UUT (such as serial number or station software version)
    ///@param inHandle : the handle originally returned from IP_UUTStart()
    IP_API_Reply IP_addAttribute( IP_UUTHandle inHandle, const char* kcpName, const char* kcpValue );
    
    ///rework and module apis for only select few non-test stations.
    ...
    IP_API_Reply IP_addBlob( IP_UUTHandle inHandle, const char* kcpBlobName, const char* kcpPathToBlobFile );
    /// Guidelines for blob retention:
    /// - 30-90 normal test blobs
    /// - Critical to Safety module and subassembly tests: 1-4years
    /// - Critical to Safety FATP FG tests: 5-10years
    ///Please sync with TDL and Apple Legal for Critical to Safety requirements. Stations not deemed “CTS” should not use this blob retention API.
    ...
    IP_API_Reply IP_addBlobWithPolicyYears( IP_UUTHandle inHandle, const char *kcpYears, const char* kcpBlobName,const char* kcpPathToBlobFile );
    ...
    IP_API_Reply IP_addBlobData( IP_UUTHandle inHandle, const char* kcpBlobName, const void* kvpBlobPointer, long lBlobLength );
    ...
    IP_API_Reply IP_addBlobDataWithPolicy( IP_UUTHandle inHandle,enum IP_ENUM_BLOBPOLICY eIPBlobPolicy, const char* kcpBlobName,
                                          const void* kvpBlobPointer, long lBlobLength );

    // UUT status functions
    
    ///Submit an AmIOkay query to the process control system
    ///@param UUTHandle : the handle originally returned from IP_UUTStart()
    IP_API_Reply IP_amIOkay( IP_UUTHandle inHandle, const char* kcpUUTSerialNumber );
    
    IP_API_Reply IP_isSignatureOK(IP_UUTHandle inHandle);

    
    ///return the error or message/data (if any)
    const char* IP_reply_getError( IP_API_Reply replyToCheck );
    ...
    void IP_reply_destroy( IP_API_Reply replyToCheck );
    
    
    
    ///returns true if message contains good news
    ///@param reply : the IP_API_Reply handle returned from an API call
    bool IP_success( IP_API_Reply replyToCheck );
    ...
    bool IP_testResult_setResult( void* vpTestResultHandle, enum IP_PASSFAILRESULT eResult );
    ...
    ///set the 'value' of the TestResult
    ///@param testResultHandle : the handle originally returned from IP_testResult_create()
    bool IP_testResult_setValue( void* vpTestResultHandle, const char* kcpValue, size_t valueLength );

    ///*************** CAUTION PLEASE NOTE************************
    ///Call IP_testResult_setValueSupplnat api ONLY if your test result has parametric values and
    ///has already called IP_testSpec_setLimitsSupplant api and build stage is NOT of the of the
    ///following stages DVT,PVT,RAMP or MP also your test has to be a parametric data test
    ///otherwise just pass/fail then DO NOT CALL THIS API
    ///set the 'valueSupplant' of the TestResult
    ///@param testResultHandle : the handle originally returned from IP_testResult_create()
    bool IP_testResult_setValueSupplant( void* testResultHandle, const char* kcpValueSupplant, size_t valueSupplantLength );

    ///set the Supplant of the TestResult
    ///@param testResultHandle : the handle originally returned from IP_testResult_create()
    bool IP_testResult_setResultSupplant( void* testResultHandle, enum IP_PASSFAILRESULT resultSupplant );

    ///set the 'value' of the TestResult
    ///@param testResultHandle : the handle originally returned from IP_testResult_create()
    bool IP_testResult_setMessage( void* testResultHandle, const char* kcpMessage, size_t messageLength );
    ...
    bool IP_testSpec_setTestName( IP_TestSpecHandle testSpecHandle, const char* kcpName, size_t nameLength );
    
    ///set the sub Test Name
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    bool IP_testSpec_setSubTestName( IP_TestSpecHandle testSpecHandle, const char* kcpName, size_t nameLength );
    
    ///set the sub-sub Test name
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    bool IP_testSpec_setSubSubTestName( IP_TestSpecHandle testSpecHandle, const char* kcpName, size_t nameLength );
    
    ///set the test limits
    ///If your parametric measurement has no limits, do not call setLimits, or use NA.
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    bool IP_testSpec_setLimits( IP_TestSpecHandle testSpecHandle,	const char* kcpLowerLimit, size_t lowerLimitLength,const char* kcpUpperLimit, size_t upperLimitLength );
    ///set the supplant test limits
    ///If your parametric measurement has no limits, do not call setLimits, or use NA.
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    bool IP_testSpec_setLimitsSupplant( IP_TestSpecHandle testSpecHandle,    const char* kcpLowerLimitSupplant, size_t lowerLimitSupplantLength,const char* kcpUpperLimitSupplant, size_t upperLimitSupplantLength );

    ///set the units that the test result is measured measured in
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    bool IP_testSpec_setUnits( IP_TestSpecHandle testSpecHandle, const char* kcpUnits, size_t unitsLength );
    ...
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    ///@param has to be the following format(struct key, val) key ->Sequence1@Key7 and val->Val7, key->Sequence2@Key2 val->Val2, key->rootKey val->rootVal
    ///"@" signifies one level deep dictionary
    /// also "@", "," and ":" should not be part of the key or value.
    bool IP_testSpec_setTestContext( IP_TestSpecHandle testSpecHandle,struct IP_kvStruct  *kvStruct[],int size);

    ///set this item to be a hi res timestamp, empty or negative or 0 will return false
    ///@param testSpecHandle : the handle originally returned from IP_testSpec_create()
    bool IP_testSpec_setStartTime( IP_TestSpecHandle testSpecHandle, time_t rawTimeToUse);
    ...
    IP_API_Reply IP_validateSerialNumber( IP_UUTHandle inHandle, const char* kcpSerialNumber );
    ...
    IP_API_Reply IP_setDUTPosition(IP_UUTHandle inHandle,enum IP_ENUM_FIXTURE_ID eFixId,enum IP_ENUM_FIXTURE_HEAD_ID eHeadId) __deprecated_msg("IP_setDUTPosition will be deprecated in next major revision");
    
    //twin api of the IP_setDUTPosition, instead of enums it takes const char * as arguments
    IP_API_Reply IP_setDUTPos(IP_UUTHandle handleDUTPosition,const char * kcpFixId,const char * kcpHeadId);
     ...
     */
    IP_API_Reply IP_getJsonResultsObj(IP_UUTHandle inHandle);
    const char* IP_reply_getJsonData( IP_API_Reply replyToCheck );
    ...
    ///add unit config item for the test
    ///@param testHandle : the handle originally returned from IP_UUTStart()
    ///@param has to be the following format(struct key, val) key ->Key7 and val->Val7, key->Key2 val->Val2, key->rootKey val->rootVal"
    IP_API_Reply IP_addUnitConfig(IP_UUTHandle ahUUTHandle,struct IP_kvStruct  *kvStruct[],int iSize);
    ///@param has to be the following format(struct key, val) key ->Key7 and val->Val7, key->Key2 val->Val2, key->rootKey val->rootVal"
    
    IP_API_Reply IP_addTestBOM(IP_UUTHandle ahUUTHandle,struct IP_kvStruct  *kvStruct[],int iSize);
