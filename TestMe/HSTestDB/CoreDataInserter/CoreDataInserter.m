...
- (void)_save{
    ...
    if ([self.moc hasChanges]) {
        if ([self.moc save:&err] == NO) {
            HSLogInfo(@"[CoreDataInserter]save failure with err:%@",[err localizedDescription]);
        }
    }
//    //读取
...
- (NSArray *)_fetchAllUnarchivedObjectsWithEntityName:(NSString *)name{
    ...
    if (resultArr == nil) {
        HSLogError(@"Error while fetching all %@: %@",name,[err localizedDescription]);
    }
    return resultArr;
}
- (id)_fetchSingleObjectWithRequest:(NSFetchRequest *)request{
    ...
    if (resultArr != nil) {
        if ([resultArr count] <= 1) {
            ...
            HSLogError(@"ERROR: More than one object found!! %@",resultArr);
        }
    }else{
        HSLogError(@"Saw error fetching single object: %@",[err localizedDescription]);
    }
    return findObj;
}
...
//       [request2 setFetchLimit:0x01];
        ...
        HSLogInfo(@"[CDI]-archivedUnits count:%ld",[archivedUnitsArr count]);
        if (archivedUnitsArr == nil || err2 != nil) {
            HSLogError(@"ERROR deleting old group data: %@",[err2 localizedDescription]);
        }else{
            for (DBUnit *unit in archivedUnitsArr) {
                HSLogInfo(@"[CDI]delete unit:%@ uuid:%@",unit.identifier,unit.uuid);
...
-(void)testItemFinished:(HSEvent *)event{
    [self.moc performBlock:^{
        ...
        if (record == nil) {
            HSLogError(@"[CoreDataInserter]ERROR:_fetchRecordWithUUID result nil!");
            //return 0;
            return;
        }
...
-(void)unitFinished:(HSEvent *)event{
    [self.moc performBlock:^{
        ...
        if (unit != nil) {
            ...
            HSLogError(@"[CoreDataInserter]unitFinished ERROR:not fetch Unit with uuid:%@",hsUnit.uuid);
        }
    }];
}
